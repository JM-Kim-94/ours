...
class PEARLSoftActorCritic(MetaRLAlgorithm):
    ...
    #################3
    def meta_train(self, train_step, pretrain=False):
        ...
        ###############################################################################################################
        ###############################################################################################################
        num_fake_tasks = 5  # dir2에서 num_fake_tasks가 1개여서 사이클의 영향이 거의 미미하게 들어가는 것 같다.
        print("num_fake_tasks", num_fake_tasks)
        ###############################################################################################################
        ###############################################################################################################
        """"""
        ...
        if self.use_fakesample_representation:
            ...
            real_task_indices = np.random.choice(self.train_tasks, num_fake_tasks, replace=True)
            ...
            obs_real1_flat, actions_real1_flat, rewards_real1_flat, next_obs_real1_flat = obs_real1.view(num_fake_tasks * self.batch_size, -1), actions_real1.view(num_fake_tasks * self.batch_size, -1), rewards_real1.view(num_fake_tasks * self.batch_size, -1), next_obs_real1.view(num_fake_tasks * self.batch_size, -1)
            obs_real2_flat, actions_real2_flat, rewards_real2_flat, next_obs_real2_flat = obs_real2.view(num_fake_tasks * self.batch_size, -1), actions_real2.view(num_fake_tasks * self.batch_size, -1), rewards_real2.view(num_fake_tasks * self.batch_size, -1), next_obs_real2.view(num_fake_tasks * self.batch_size, -1)
            obs_real_on_flat, actions_real_on_flat = obs_real_on.view(num_fake_tasks * self.embedding_batch_size, -1), actions_real_on.view(num_fake_tasks * self.embedding_batch_size, -1)
