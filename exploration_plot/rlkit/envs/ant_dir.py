...
#         train_goal_dir = [i * np.pi / 2 for i in range(4)]
#         eval_goal_dir = [i * np.pi / 2 + np.pi / 4 for i in range(4)]
#         tsne_goal_dir = [i * np.pi / 4 for i in range(8)]  # train_goal_dir + eval_goal_dir
...
class AntDirEnv(AntEnv):
    def __init__(self, num_train_tasks=2, eval_tasks_list=[], indistribution_train_tasks_list=[], TSNE_tasks_list=[],
                        ood_type='inter', done_flase=False, use_ref_task=0, expert=False):
        self.expert = expert
        self._task = {}
    ...
    def sample_tasks(self, num_train_tasks, eval_tasks_list, indistribution_train_tasks_list, TSNE_tasks_list, 
        ...
        if ood_type == "inter":
            if self.expert:
                train_goal_dir = [0.0, 0.5 * np.pi, np.pi, 1.5 * np.pi] + [0.25 * np.pi, 0.75 * np.pi, 1.25 * np.pi, 1.75 * np.pi]
            else:
            ...
            if self.expert:
                train_goal_dir = [0.0, 0.5 * np.pi] + [3 * np.pi / 4,    7 * np.pi / 4] + [1 * np.pi / 4]
            else:
            ...
            tsne_goal_dir  = [0.0 * np.pi / 4,
                              ...
                              3.0 * np.pi / 4,
                              7.0 * np.pi / 4]  # 5개
            train_tsne_goal_dir, test_tsne_goal_dir = [], []
            


        goal_dirs = train_goal_dir + eval_goal_dir + indistribution_goal_dir + tsne_goal_dir + train_tsne_goal_dir + test_tsne_goal_dir
